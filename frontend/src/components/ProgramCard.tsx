...
import { Globe, CreditCard, Clock, Languages } from 'lucide-react';
...
const ProgramCard: React.FC<ProgramCardProps> = ({ program }) => {
  ...
  return (
    <div className="program-card">
      ...
      <p className="program-card-info" style={{ display: 'flex', alignItems: 'center', gap: 4 }}><Globe size={13} strokeWidth={1.75} /> {program.country}{program.city ? ` / ${program.city}` : ''}</p>
      <p className="program-card-info" style={{ display: 'flex', alignItems: 'center', gap: 4 }}><CreditCard size={13} strokeWidth={1.75} /> {formatTuition(program.tuition)}</p>

      {program.duration && (
        <p className="program-card-info" style={{ display: 'flex', alignItems: 'center', gap: 4 }}><Clock size={13} strokeWidth={1.75} /> {program.duration}</p>
      )}
      {program.language && (
        <p className="program-card-info" style={{ display: 'flex', alignItems: 'center', gap: 4 }}><Languages size={13} strokeWidth={1.75} /> {program.language}</p>
      )}
    </div>
  );
};
