...
const ChatWidget = () => {
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      setMessages([
        {
          role: 'assistant',
          content: 'Veuillez vous connecter pour poser vos questions au chatbot.',
        },
      ]);
      setSuggestions([]);
  ...
  }, [inputValue, isAuthenticated, isLoading, messages, isOpen]);
      ...
      {(isOpen || isClosing) && (
        <div className={`chat-window ${isClosing ? 'closing' : ''}`}>
          ...
          <div className="chat-header">
            <div>
              <div className="chat-header-title" style={{ display: 'flex', alignItems: 'center', gap: 6 }}><MessageSquare size={16} strokeWidth={1.75} /> Orientus</div>
          ...
          <div className="chat-body" role="log" aria-live="polite">
            {!isAuthenticated ? (
              <div style={{ padding: '14px' }}>
                <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '12px', padding: '14px' }}>
                  ...
                  <p style={{ margin: '8px 0 12px', color: '#64748b', fontSize: '0.92rem' }}>
                    Il faut se connecter pour repondre a vos questions.
                  </p>
                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                    <Link
                      to="/login"
                      onClick={toggleChat}
                      style={{ background: '#2563eb', color: '#fff', borderRadius: '10px', padding: '8px 12px', fontWeight: 600, textDecoration: 'none' }}
                    >
                      Se connecter
                    </Link>
                    <Link
                      to="/register"
                      onClick={toggleChat}
                      style={{ border: '1px solid #cbd5e1', color: '#334155', borderRadius: '10px', padding: '8px 12px', fontWeight: 600, textDecoration: 'none' }}
                    >
                      Créer un compte
                    </Link>
                  </div>
              ...
              messages.map((msg, index) => {
                const isLastBotError =
                  ...
                  msg.content.startsWith('Désolé, une erreur');
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <RefreshCw size={13} strokeWidth={1.75} /> Réessayer
                    </button>
                    )}
                  </ChatBubble>
                );
            ...
            {isAuthenticated && isLoading && <TypingIndicator />}
