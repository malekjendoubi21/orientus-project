...
import {
  BookOpen, Globe, BarChart2, CreditCard, Target, Award,
  Trophy, Sparkles,
  AlertTriangle, Search, Cpu, XCircle, ChevronLeft, ChevronRight,
} from 'lucide-react';
...
// ─── Wizard form steps ─────────────────────────────────
const WIZARD_STEPS = [
  { id: 1, title: 'Vos études', Icon: BookOpen },
  { id: 2, title: 'Préférences', Icon: Globe },
  { id: 3, title: 'Compétences', Icon: BarChart2 },
  { id: 4, title: 'Budget', Icon: CreditCard },
];
  ...
  const renderForm = () => (
    ...
    >
      ...
      <div className="text-center mb-8">
        ...
        >
          <Target size={30} strokeWidth={1.75} className="text-white" />
        </motion.div>
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-6 md:p-8">
        <AnimatePresence mode="wait">
          ...
          {wizardStep === 1 && (
            ...
            >
              <h2 className="text-lg font-semibold text-gray-800 flex items-center gap-2">
                <BookOpen size={18} strokeWidth={1.75} className="text-blue-600" /> Vos études
              </h2>
              <SelectField label="Domaine d'intérêt" field="interestField" options={INTEREST_FIELDS} required />
              <SelectField label="Diplôme visé" field="targetDegree" options={TARGET_DEGREES} required />
              <SelectField label="Diplôme actuel" field="currentDegree" options={CURRENT_DEGREES} required />
          ...
          {wizardStep === 2 && (
            ...
            >
              <h2 className="text-lg font-semibold text-gray-800 flex items-center gap-2">
                <Globe size={18} strokeWidth={1.75} className="text-blue-600" /> Vos préférences
              </h2>
              <SelectField label="Pays préféré" field="preferredCountry" options={COUNTRIES} />
              <SelectField label="Langue préférée" field="preferredLanguage" options={LANGUAGES} />
              <SelectField label="Mode d'études" field="studyMode" options={STUDY_MODES} />
            </motion.div>
          )}
          ...
          {wizardStep === 3 && (
            ...
            >
              <h2 className="text-lg font-semibold text-gray-800 flex items-center gap-2">
                <BarChart2 size={18} strokeWidth={1.75} className="text-blue-600" /> Compétences linguistiques
              </h2>
              <SelectField label="Niveau de langue" field="languageLevel" options={LANGUAGE_LEVELS} />
              <div>
                  <label className="block text-sm font-medium text-gray-600 mb-1.5">
          ...
          {wizardStep === 4 && (
            ...
            >
              <h2 className="text-lg font-semibold text-gray-800 flex items-center gap-2">
                <CreditCard size={18} strokeWidth={1.75} className="text-blue-600" /> Budget & financement
              </h2>
              <div>
                <label className="block text-sm font-medium text-gray-600 mb-1.5">
        ...
        >
          ...
          {wizardStep < 4 ? (
            ...
            >
              <Search size={16} strokeWidth={1.75} /> Trouver mes programmes
            </motion.button>
          )}
        </div>
      </div>
    ...
    >
      ...
      <div className="relative inline-flex items-center justify-center mb-8">
        ...
        <div className="relative w-16 h-16 bg-gradient-to-br from-blue-500 to-blue-700 rounded-full flex items-center justify-center shadow-lg">
          <Cpu size={28} strokeWidth={1.75} className="text-white" />
        </div>
      </div>
    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
        <div>
          <h2 className="text-2xl md:text-3xl font-bold text-gray-900 flex items-center gap-2">
            <Trophy size={26} strokeWidth={1.75} className="text-blue-600" /> Vos programmes recommandés
          </h2>
          <div className="flex items-center gap-3 mt-2">
            ...
            {mlAvailable && (
              <span className="inline-flex items-center gap-1 bg-green-50 text-green-700 text-xs font-medium px-2.5 py-1 rounded-full border border-green-100">
                <Sparkles size={11} strokeWidth={1.75} /> Recommandation IA
              </span>
            )}
          </div>
        </div>
      ...
      {!mlAvailable && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-6 flex items-start gap-3">
          <AlertTriangle size={20} strokeWidth={1.75} className="text-yellow-500 shrink-0 mt-0.5" />
          <div>
      ...
      {results.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-2xl shadow-lg border border-gray-100">
          <div className="inline-flex items-center justify-center w-20 h-20 bg-gray-100 rounded-full mb-4">
            <Search size={36} strokeWidth={1.5} className="text-gray-400" />
          </div>
                ...
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {rec.degree && (
                    <span className="inline-flex items-center gap-1 text-xs bg-blue-50 text-blue-700 px-2.5 py-1 rounded-full font-medium">
                      <Award size={10} strokeWidth={1.75} /> {rec.degree}
                    </span>
                  )}
                  {rec.category && (
                    <span className="inline-flex items-center gap-1 text-xs bg-gray-100 text-gray-600 px-2.5 py-1 rounded-full">
                      <BookOpen size={10} strokeWidth={1.75} /> {rec.category}
                    </span>
                  )}
                  {rec.language && (
                    <span className="inline-flex items-center gap-1 text-xs bg-gray-100 text-gray-600 px-2.5 py-1 rounded-full">
                      <Globe size={10} strokeWidth={1.75} /> {rec.language}
                    </span>
                  )}
                </div>

                {/* Tuition */}
                <p className="text-gray-700 text-sm font-medium mb-3 flex items-center gap-1"><CreditCard size={13} strokeWidth={1.75} className="text-gray-400" /> {formatTuition(rec.tuition)}</p>
    ...
    >
      <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8 md:p-10 text-center">
        ...
        <p className="text-gray-600 mb-8">
          Vous devez avoir un compte pour utiliser les recommandations personnalisées par IA.
        </p>

        <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
          <Link
            to="/login"
            className="w-full sm:w-auto px-6 py-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white font-semibold rounded-xl hover:from-blue-700 hover:to-blue-800 transition-all shadow-lg"
          >
            Se connecter
          </Link>
          <Link
            to="/register"
            className="w-full sm:w-auto px-6 py-3 border-2 border-gray-200 text-gray-700 font-semibold rounded-xl hover:border-blue-500 hover:text-blue-600 transition-all"
          >
            Créer un compte
          </Link>
        </div>
